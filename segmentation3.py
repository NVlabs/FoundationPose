...
class MaskGenerator:
    ...
    def get_points(self, image):
        ...
        def click_event(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                cv2.imshow(f'{self.mesh_name}_{self.frame_id+4}', image)

        cv2.imshow(f'{self.mesh_name}_{self.frame_id+4}', image)
        cv2.setMouseCallback(f'{self.mesh_name}_{self.frame_id+4}', click_event)
        cv2.waitKey(0)
    ...
    def create_mask_and_display_result(self):
        ...
        cv2.imshow(f'{self.mesh_name}_{self.frame_id+4}_mask', mask)
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # Save the mask using OpenCV
        mask_path = f"{self.base_dir}/masks/{self.mesh_name}_mask_{self.frame_id+4}.png"
    ...
    def generation(self):
        # Load the image
        self.image = Image.open(self.color_files[self.frame_id+4])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_scene_dir', type=str, default=f'{code_dir}/perception_data/green_paper/0')
